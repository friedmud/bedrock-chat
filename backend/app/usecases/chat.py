...
def chat(
    ...
    if is_tool_use_supported(chat_input.message.model):
        tools = get_tools(bot)
    ...
    if bot is not None:
        if bot.is_agent_enabled() and is_tool_use_supported(chat_input.message.model):
        ...
        elif bot.has_knowledge() and not is_tool_use_supported(chat_input.message.model):
